...
- 10分钟模块，按名称浏览启示录课程，显示完整标题，并按核心、绪论、本论、结论分组展示文档内容。
- 管理后台，支持用户和上传文件管理。
...
所有线上环境都需要执行最新的 D1 migration。当前版本新增了页面标题尺寸字段：

```sql
ALTER TABLE notes ADD COLUMN title_size TEXT NOT NULL DEFAULT 'h1';
```

如果你从旧版本升级，执行 `pnpm db:migrate` 后，旧页面会默认使用一级标题。
